...
/**
 * SPOJ 694 - Distinct Substrings Created by Darren on 14-7-29. Solved with
 * suffix array. Observation: A substring of S is a prefix of some suffix of S.
 * After sorting the suffix array sa, the number of distinct substrings
 * contributed by the suffix s[sa[i],n-1] is n-sa[i]-lcp[i], where lcp[i] is the
 * length of the longest common prefix between s[sa[i-1],n-1] and s[sa[i],n-1].
 */
public class DistinctSubstrings {
	...
	void solve() throws IOException {
		...
		Integer[] suffixArray = new Integer[n]; // Integer instead of int for
												// using Arrays.sort()
		for (int i = 0; i < n; i++)
			suffixArray[i] = i;
		...
		// The number of distinct substrings is the sum of the difference
		// between the length of
		// each suffix and its corresponding lcp with previous suffix. This is
		// because all
	 ...
	 *
	 * @return an array containing the lengths of longest common prefix for the
	 *         given suffix array.
	 */
