...
public static void main(String[] args) throws IOException {
		...
		SegmentTree2D st = new SegmentTree2D(mat,n);
	...
	static class SegmentTree2D {
		...
		int[] query(int node, int i, int j, int len, int a_start, int b_start, int a_end, int b_end) {
			if (a_start >= i + len || b_start >= j + len || a_end < i || b_end < j)
				return new int[] { 0, INF };

			if (a_start <= i && b_start <= j && a_end >= i + len - 1 && b_end >= j + len - 1)
			...
			return new int[] { Math.max(Math.max(q1[0], q2[0]), Math.max(q3[0], q4[0])),
					Math.min(Math.min(q1[1], q2[1]), Math.min(q3[1], q4[1])) };
		}
	}
}
